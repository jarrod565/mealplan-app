...
- 🔄 CB_12 — Bring Your Own Recipes (Airtable — v1)
  - Airtable OAuth connection and token storage
  - Base and table selection with auto-detected column mapping
  - Live preview card for user confirmation of column mapping
  - For You filter drawer — toggle active sources, link to Manage Connections
  - Settings → Connections screen for all connected sources
  - Airtable cards served via CB_09 adapter pattern with CB_10 extraction pipeline
  - Dynamic For You empty state with connection CTAs
  - Google Sheets deferred to future enhancement
...
**Connected Sources (future beyond current):**
- Google Sheets integration (CB_12 future enhancement — deferred to avoid Google OAuth approval overhead)
- AnyList integration
...
| CB_12_BYOR.md | Airtable OAuth, column mapping preview, For You filter drawer, BYOR card adapter | 🔄 In Progress |
